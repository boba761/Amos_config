            ...
            { 
                ...
                if ( _useDwellTime )
                {
                    if ( Table.TypeTable == TableSequence.typeTable2D )
                        _increment = Program.Document.Dashboard["Dwell 2D"];
                    if ( Table.TypeTable == TableSequence.typeTable3D )
                        _increment = Program.Document.Dashboard["Dwell 3D"];
                    if ( Table.TypeTable == TableSequence.typeTable4D )
                        _increment = Program.Document.Dashboard["Dwell 4D"];
                    else
                        _increment = Program.Document.Dashboard["Dwell Time"];
                }
            }
        }
            ...
            set
            {
                ...
                else if ( Regex.IsMatch( text, @"^=\s*[a-z][a-z0-9\s\.,]*$", RegexOptions.IgnoreCase ) )
                {
                    if ( Table == null )
                        ...
                        _increment = Program.Document.Sequence.CheckTableVariable( new TableVariable( text.Substring( 1 ).Trim( ), new TimeValue( 0 ),
