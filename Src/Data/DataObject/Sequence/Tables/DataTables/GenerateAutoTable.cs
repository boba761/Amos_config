    ...
    public class GenerateAutoTable : ICloneable
    {
        ...
        public string Start
        {
            ...
            set
            {
                ...
                {
                    if ( Table == null )
                        _start = Document.LoadDocument.Sequence.GetTableVariable( text.Substring( 1 ).Trim( ) );
                    else
                        _start = Document.LoadDocument.Sequence.CheckTableVariable( new TableVariable( text.Substring( 1 ).Trim( ), new DoubleValue( 0 ),
            ...
            set
            {
                ...
                else if ( Regex.IsMatch( text, @"^=[a-z][a-z0-9\s\.,]*$", RegexOptions.IgnoreCase ) )
                {
                    if ( Table == null )
                        ...
                        _increment = Document.LoadDocument.Sequence.CheckTableVariable( new TableVariable( text.Substring( 1 ).Trim( ), new DoubleValue( 0 ),
                            new DoubleValue( -100 ), new DoubleValue( 100 ) ) );
                }
                else
                    throw new Exception( "In the field Start must be or variable or value." );
        ...
        public string Number
        {
            ...
            set
            {
                ...
                {
                    if ( Table == null )
                        _number = Document.LoadDocument.Sequence.GetTableVariable( text.Substring( 1 ).Trim( ) );
                    else
                        _number = Document.LoadDocument.Sequence.CheckTableVariable( new TableVariable( text.Substring( 1 ).Trim( ), new IntegerValue( 0 ),
                            new IntegerValue( 0 ) ) );
                }
                else
                    throw new Exception( "In the field Start must be or variable or value." );
