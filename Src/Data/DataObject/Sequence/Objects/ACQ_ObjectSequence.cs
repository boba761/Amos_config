...
{
    public class ACQ_ObjectSequence : ObjectStream
    {
        ...
        public override void CorrectionSequence( ICommandList command )
        {
            ...
            if ( Next != null && Next.IsChange == false )
            {
                ...
                {
                    Next.IsChange = true;
                    command.Add( new SetObjectCommand( ( (Command)command ).Document, Parent, TypeCollection, Next.IndexEvent, eTypeObjectStream.Acquisition, 
                        ( (ObjectStream)Old ).Acquisition ) );
                }
            }
        }
